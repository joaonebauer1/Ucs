...
public class Class_LevelEasy extends View implements MediaPlayer.OnCompletionListener {
    ...
    public Class_LevelEasy(Context context) {
        super( context );
    }
    ...
    public Class_LevelEasy(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_MOVE) {
            ...
            if (absDx > cellSize || absDy > cellSize) {
                if (absDx > absDy) {
                    // move-se na direção X
                    if (dx > 0)
                        movePlayer(Direction.RIGHT);
                    else
                        movePlayer(Direction.LEFT);
                } else {
                    // move-se na direção Y
                    if (dy > 0)
